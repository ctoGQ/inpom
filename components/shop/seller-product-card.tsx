...
export function SellerProductCard({
  ...
  return (
    <Link href={`/mycabinet/shop/${id}`}>
      <div className="group relative overflow-hidden rounded-2xl border border-foreground/10 hover:border-primary/50 transition-all hover:shadow-lg h-full flex flex-col bg-card hover:bg-card/80 cursor-pointer active:scale-95">
        {/* Image container */}
        ...
        <div className="flex-1 flex flex-col p-4">
          ...
          {/* Info text */}
          <p className="text-xs text-primary font-medium group-hover:underline mt-auto">
            Клацніть для перегляду та редагування →
          </p>
        </div>
      </div>
    </Link>
